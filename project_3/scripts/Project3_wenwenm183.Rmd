...
```
  ...
  The outliers are mainly from clients who have employer provided health insurance. However, it is important to note that majority of the clients have employer provided health insurance if they have have insurance. 
```{r, echo=FALSE, message=FALSE, warning=FALSE}
...
```
  ...
  The outliers are from both female and male clients of UMD. Therefore, we can't conclude that gender is associated with number of visits. 
...
```
  ...
  From the boxplots, we can see that most of the outliers are from year 2014 and on. It seems that there is a trend that the number of clients who frequently visit UMD is decreasing starting from 2016. So I suspect that there is some correlation between the recent years and the number of visit. 
...
```
From the scatterplot, we can see that number of visits have decreased over the years. however more analysis is necessary to say if this decrease in the number of visits is significant. 
...
```
  ...
  In order to fit a model, we have to see if there are any outliers, which there are some shown in the boxplots above. The density for the year is somewhat normally distributed. Since N is large, we can proceed to fit a model to determine if the decreasing trend is significant. 
